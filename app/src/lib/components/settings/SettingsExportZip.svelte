...
<Section title={m.settings_export_zip_title()} icon={Archive} collapsible defaultOpen={false}>
	<p class="m-0 font-sans text-xs text-subtle-foreground italic">
		{m.settings_export_zip_hint()}
	</p>
	<Button
		variant="secondary"
		icon={Archive}
		...
		fullWidth
	>
		{exportingZip ? m.settings_export_zip_exporting() : m.settings_export_zip_btn()}
	</Button>
</Section>
