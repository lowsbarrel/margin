<script lang="ts" module>
	...
	export type ButtonProps = WithElementRef<HTMLButtonAttributes> &
		WithElementRef<Omit<HTMLAnchorAttributes, 'href'>> & {
			...
			size?: ButtonSize;
			/**
			 * Narrower than `HTMLAnchorAttributes['href']`: rendering an <a> here is
			 * an internal navigation, so the target has to come out of SvelteKit's
			 * `resolve()` rather than be an arbitrary string. For an off-app link,
			 * use a plain <a> with `rel="external"`.
			 */
