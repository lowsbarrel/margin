...
export async function listSnapshots(vaultPath: string, filePath: string): Promise<Snapshot[]> {
...
export async function deleteSnapshot(
	...
	const r = await commands.deleteSnapshot(vaultPath, filePath, snapshotFilename);
	if (r.status === 'error') throw r.error;
}

export async function clearSnapshots(vaultPath: string, filePath: string): Promise<number> {
