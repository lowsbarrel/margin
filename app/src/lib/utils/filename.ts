...
export function validateName(name: string): string | null {
	...
	if (NON_ASCII.test(trimmed)) return m.validate_name_ascii();
	if (INVALID_CHARS.test(trimmed)) return m.validate_name_invalid_chars();
	...
	if (RESERVED_NAMES.has(stem.toUpperCase())) return m.validate_name_reserved({ name: trimmed });
	return null;
}
