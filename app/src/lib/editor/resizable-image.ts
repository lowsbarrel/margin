...
export const ResizableImage = Image.extend({
	...
	addNodeView() {
		return ({ node, editor, getPos }) => {
			...
			handleLeft.classList.add('image-resize-handle', 'image-resize-handle-left');
			handleLeft.contentEditable = 'false';

			const handleRight = document.createElement('div');
			handleRight.classList.add('image-resize-handle', 'image-resize-handle-right');
			...
			for (const { value, icon } of alignments) {
				...
				btn.addEventListener('mousedown', (e) => {
					...
					editor
						.chain()
						.focus()
						.setImageAlign(value as 'left' | 'center' | 'right')
						.run();
				});
			...
			function updateToolbarVisibility() {
				...
				const currentAlign = container.getAttribute('data-align') || 'center';
				toolbar.querySelectorAll<HTMLButtonElement>('.image-align-btn').forEach((btn) => {
			...
			function onMouseMove(e: MouseEvent) {
				...
				const maxWidth = containerEl ? containerEl.clientWidth : window.innerWidth;
			...
			const selectionHandler = () => {
				...
				const pos = getPos();
				if (pos == null) return;
				const isNodeSelection = selection instanceof NodeSelection && selection.from === pos;
			...
			return {
				...
				update: (updatedNode: PMNode) => {
					...
					(node as { attrs: typeof updatedNode.attrs }).attrs = updatedNode.attrs;
