	...
	addStorage() {
		return {
			markdown: {
				...
				parse: {
					setup(this: { options: FileEmbedOptions }, md: MarkdownIt) {
...
function wikiFileEmbedPlugin(md: MarkdownIt, attachmentFolder: string) {
	md.inline.ruler.push('wiki_file_embed', function (state, silent) {
		...
		const fileSrc = filename.includes('/') ? filename : `${attachmentFolder}/${filename}`;
	...
	md.renderer.rules.file_embed = function (tokens, idx) {
