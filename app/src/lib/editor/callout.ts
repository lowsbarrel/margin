...
interface MdBlockState {
	...
	lineMax: number;
	parentType: string;
	md: { block: { tokenize(state: MdBlockState, startLine: number, endLine: number): void } };
...
const VALID_TYPES: CalloutType[] = ['info', 'note', 'success', 'warning', 'danger'];

function getValidCalloutType(value: string | null): CalloutType {
	if (value && VALID_TYPES.includes(value as CalloutType)) return value as CalloutType;
	return 'info';
}
...
function findCalloutDepth($pos: ResolvedPos): number | null {
	for (let d = $pos.depth; d > 0; d--) {
