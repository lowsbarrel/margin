...
import { writeFileBytes, fileExists, createDirectory, importExternalFile } from '$lib/fs/bridge';
...
export async function insertPastedFile(
	...
	insertIntoEditor(editor, relPath, file.name, isImageFile(file.name, file.type), vaultPath);
}
...
function insertIntoEditor(
	...
	if (isImage) {
		...
		editor.chain().focus().setFileEmbed({ src: relPath, filename: displayName }).run();
	}
}
