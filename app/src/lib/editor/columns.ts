		...
		return {
			...
			Backspace: ({ editor }) => {
				...
				if (
					...
					tr.replaceWith(columnsPos, columnsPos + columns.nodeSize, Fragment.from(content));
...
export const Columns = Node.create({
	...
	renderHTML({ HTMLAttributes, node }) {
		...
		return ['div', mergeAttributes(HTMLAttributes, { 'data-columns': String(cols) }), 0];
	},
	...
	addCommands() {
		return {
			insertColumns:
				({ cols }) =>
				({ commands }) => {
					const layout = cols === 3 ? 'three_equal' : 'two_equal';
