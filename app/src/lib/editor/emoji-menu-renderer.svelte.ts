...
export default function renderEmojiMenu() {
  let wrapper: HTMLDivElement | null = null;
  let items = $state<EmojiItem[]>([]);
  let selectedIndex = $state(0);
  let command: ((item: EmojiItem) => void) | null = null;
  ...
  return {
    onStart: (props: any) => {
      items = getEmojiItems({ query: props.query });
      selectedIndex = 0;
      ...
      component = mount(EmojiMenu, {
        ...
        props: {
          ...
          onselect: (item: EmojiItem) => command?.(item),
