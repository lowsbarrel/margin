...
export const Details = Node.create({
	...
	renderHTML({ HTMLAttributes }) {
		return ['details', mergeAttributes({ class: 'details-block' }, HTMLAttributes), 0];
	},
...
export const DetailsSummary = Node.create({
	...
	renderHTML({ HTMLAttributes }) {
		return ['summary', mergeAttributes({ class: 'details-summary' }, HTMLAttributes), 0];
	},
					...
					if ($from.node(d).type.name === 'detailsContent') {
						...
						if (
							...
							if ($from.node(detailsDepth)?.type.name === 'details') {
								...
								const tr = state.tr.delete(emptyBlockPos, emptyBlockEnd);
								tr.insert(tr.mapping.map(afterDetails), state.schema.nodes.paragraph.create());
...
function detailsMarkdownPlugin(md: MarkdownIt) {
	md.block.ruler.before('fence', 'details', function (state, startLine, endLine, silent) {
